...
class `13548` {
    ...
    fun solution() {
        val N = readln().toInt().also { Query.nSqrt = sqrt(it.toDouble()).toInt() }
        val A = readln().split(" ").map(String::toInt)
        val M = readln().toInt()
        val Q = List(M) { i -> readln().split(" ").map { it.toInt() - 1 }.let { Query(it[0], it[1], i) } }
            .sorted()
